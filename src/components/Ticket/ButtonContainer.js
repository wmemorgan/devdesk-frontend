...
export const ButtonContainer = props => {
  const showAssignButton = () => {
    let show =
      !props.ticket.closed && props.ticket.opened_by !== props.activeUser.id;
  ...
  const showAddCommentButton = () => {
    return (
      (props.assigned && props.assignedTo === props.activeUser.id) ||
      props.ticket.opened_by === props.activeUser.id
    );
  };
